...
A repository for our department could be used to store not only a Digital Skills Primer resource but also code developed by staff (for instance, scripts I developed to make API calls, automate data cleaning and calculation of moving averages and perform web scraping). This means that the code would be accessible to everyone to learn from, use and adapt for their own purposes.

GitHub hosting also means anybody can contribute to code or the proposed Digital Skills Primer making this document one that can evolve and change over time and one that is owned by everybody.

\textbf{What do we need?}

A GitHub account. GitHub accounts are free but require an e-mail address to create. There are no real downsides to having an account and currently we are signed up for Flourish, Genially and Survey Monkey.
