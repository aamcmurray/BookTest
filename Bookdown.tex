...
Currently, this resource details statistics to serve as an example of the type of dynamic content that can be created by hosting and publishing through GitHub. Flicking through the pages you will see images, plots generated with R code, embedded Genially content, mathematical formula\ldots etc.

While this resource currently is based on a recent statistics course it could be developed into a \textbf{Digital Skills Primer} resource providing an introduction to \textbf{Statistics, Microsoft Power BI, Python, R} and other tools being used in research.

This Primer would serve as a singular point of reference for digital tools currently being used for research and data visualisation. This could be useful for current staff working with these tools and new staff, temporary staff and students who may not have received training on these tools. There is a wealth of documentation being developed for accessibility and style but no singular point of reference for the tools which are to be made accessible and stylised!

The aim would not be to produce a prescriptive document detailing how users must use these products but instead provide a resource with information, tutorials, worked examples, summaries and answers to frequently asked questions that would take users from zero knowledge to proficiency with the digital tools being used in research. Now is also an ideal time to develop something like this and from various conversations there is significant interest in this.

\textbf{Why host on GitHub?}

As the resource is hosted on GitHub it is available anywhere, any time and on any device (try opening it in a browser on your phone) facilitating \textbf{mobile working}. In addition, users with a GitHub account can `fork' the repository to look at the code used to generate this resource and build and improve on it in their own way or suggest changes facilitating greater \textbf{collaboration} and \textbf{transparency}.

GitHub does not need to be used to produce documentation, it can also host stand alone scripts written in any programming language which other users can fork or suggest changes to. The UK Government has several GitHub accounts, the most prominent being the \href{https://github.com/alphagov}{Government Digital Service GitHub} which is used to store and collaborate on code used to build platforms, products and services. Repositories hold all sorts of useful scripts. For instance, this introduction to \href{https://github.com/alphagov/govuk-data-science-workshop/blob/main/notebooks/2021_09_explore_govuk_structural_network.ipynb}{visualising GOV.UK as a network} or this repository built to produce \href{https://github.com/ukgovdatascience/eesectors}{reproducible economics reporting}. As a final example from my personal GitHub: a \href{https://github.com/aamcmurray/python-projects/blob/master/Examples/Curve\%20Fitting.ipynb}{curve fitting script}.
...
In addition, this resource is capable of embedding Power BI dashboards, Python and R coded visuals as well as visual elements produced in Flourish and Genially. Technically, multi-page reports with interactive visuals and content are possible meaning innovations in the types of research products produced are possible.

A repository for our department could be used to store not only a Digital Skills Primer resource but also code developed by staff (for instance, scripts I developed to make API calls, automate data cleaning and calculation of moving averages and perform web scraping). This means that the code would be accessible to everyone to learn from, use and adapt for their own purposes. It could also make collaboration with those outside our organisation more effective.

In essence, GitHub hosting means anybody can contribute to code in the repository or to the proposed Digital Skills Primer making this document one that can evolve and change over time and one that is owned by everybody decentralising knowledge and ownership.

\textbf{What do we need to develop this further?}

A GitHub account. For demonstration purposes this has been produced using a personal GitHub account however GitHub accounts are free and require only an e-mail address to create. There are no real downsides to having an account and currently we are signed up for Flourish, Genially and Survey Monkey.
