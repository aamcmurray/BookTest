...
This resource has been written using \textbf{R}, \textbf{LaTeX} and \textbf{R Studio} and published through \textbf{GitHub}. This resource supports the embedding of images, video content, html content (iframes), live code and mathematical formula and makes sharing the content much easier. In addition, this resource is capable of embedding Power BI dashboards, Python and R coded visuals.

Currently, this resource details a recent statistics course and Power BI to serve as an example of the type of dynamic content that can be created.

This resource could be developed further to produce a \textbf{Digital Skills Primer} resource providing an introduction to \textbf{Statistics, Microsoft Power BI, Python, R} and other tools being used in research. This could be useful for current staff working with these tools, new staff, temporary staff and students who may not have received training on these tools. There is a wealth of documentation being developed for accessibility and style but no single point of reference for the tools which are to be made accessible and stylised.

The aim would be to produce a resource with information, tutorials, worked examples, summaries and answers to frequently asked questions that would take users from zero knowledge to proficiency with the digital tools being used in research. Now is an ideal time to develop such a resource and from various conversations there is significant interest in this.

\textbf{Why host on GitHub?}

As the resource is hosted on GitHub it is available anywhere, any time and on any device facilitating \textbf{mobile working}.
...
A repository for our department could be used to store not only a Digital Skills Primer resource like this one but also code (for instance, scripts developed to make API calls, automate data cleaning and the calculation of moving averages and perform web scraping). This means that the code would be accessible to everyone to learn from and use and adapt for their own purposes. It could also make collaboration with those outside our organisation more effective.
...
\hypertarget{Digital-Skills-Primer}{%
\chapter*{Digital Skills Primer}\label{Digital-Skills-Primer}}
\addcontentsline{toc}{chapter}{Digital Skills Primer}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{Overview}

This resource is intended to act as a centralised resource for Digital Skills and new technologies being used in research.
...
\section{Population Data}\label{population-data}}
